...
- [x] `spec/079_PUBLIC_ROADMAP_PAGE.md` — a `/roadmap` page on the site (`src/pages/roadmap.astro` + nav + footer), site-only, no language/runtime change. Forward-only as a vertical timeline (no "recently shipped"), no dates, with a "direction, not a promise" lead. **Next**: MCP server; native Windows support (no WSL); cloud-native messaging (Azure Service Bus, AWS SQS/SNS and Amazon MQ as a future target, Google Cloud Pub/Sub and Cloud Tasks); schema field defaults; more relational providers (MySQL, and SQLite for zero-setup local development). **Exploring**: scheduled tasks, richer auth, observability.
...
- [x] **Runtime emission**: `marreta init` writes `AGENTS.md` + `.github/copilot-instructions.md` (`--no-agents` opts out); new `marreta agents` regenerates; `doctor` flags a stale stamp read-only. The primer is baked via `include_str!` and the version is injected at emission, so the committed asset stays version-neutral. Snippets are sourced from a provider-free `docs/examples/agents_primer` (scenario-tested, 6/6) plus `smart_inventory`'s functionally tested pipeline line (a pipeline is not scenario-mockable). The Cursor pointer was dropped (Cursor reads `AGENTS.md` natively); the Copilot pointer stays.
- [x] **Anti-drift gate** (`tests/agents_gate.rs` + `build.yml`): catalog ⊆ `llms-full` over all four catalog kinds (operations matched by bare name, the completeness fix from code review), every `SUMMARY` page summarized with no orphan, snippet provenance by region marker, and codegen freshness (`git diff`) plus the `agents_primer` scenarios.
- [x] **Site + docs + editor**: `sync-docs.sh` serves `llms.txt`/`llms-full.txt` at the site root and bundles the primer for the initializer (byte-parity via the regenerated init fixtures); a "Use AI assistants" how-to + CLI reference entry; a `marreta agents` VS Code palette command (extension 0.2.21). Two review gates passed (design + code review approved). Gates green: `fmt`, `clippy -D warnings`, full suite + `agents_gate`, `functional_tests`, `migrations_functional`, `e2e`, `vsce package`; site fidelity + build.
