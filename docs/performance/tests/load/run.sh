...
# --- 2. Start marreta server ---
echo "[2/7] Starting marreta-ecommerce..."
docker compose -f "$SCRIPT_DIR/docker-compose.yml" up -d marreta

# --- 3. Wait for healthcheck ---
echo "[3/7] Waiting for server to be healthy..."
ATTEMPTS=0
until [ "$(docker inspect --format='{{.State.Health.Status}}' marreta-ecommerce 2>/dev/null)" = "healthy" ]; do
  ATTEMPTS=$((ATTEMPTS + 1))
  if [ "$ATTEMPTS" -ge 30 ]; then
    echo "ERROR: Server did not become healthy after 90s."
    docker compose -f "$SCRIPT_DIR/docker-compose.yml" logs marreta
...
SLEEP_VALUE=$(grep -oP 'sleep\(\K[0-9.]+' "$SCRIPT_DIR/k6/scenarios.js" | head -1)
cat > "$RUN_DIR/config.json" <<EOF
{
  ...
  "scenarios_script": "tests/load/k6/scenarios.js",
  "thresholds_script": "tests/load/k6/thresholds.js"
}
EOF
...
bash "$SCRIPT_DIR/collect_stats.sh" "$RUN_DIR/stats.csv" &
STATS_PID=$!
cleanup() {
...
docker run --rm \
  --network "$(docker inspect marreta-ecommerce --format='{{range .NetworkSettings.Networks}}{{.NetworkID}}{{end}}' | head -1)" \
  -v "$SCRIPT_DIR/k6:/scripts" \
  -v "$RUN_DIR:/results" \
  -e BASE_URL=http://marreta-ecommerce:8080 \
